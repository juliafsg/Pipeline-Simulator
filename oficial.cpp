   	...
   	for(int i = 1; i < n_instrucoes; i++){
		...
		for(int j = 1; j<=i ; j++){
			...
			if (linha[j].opcode!= "sw"){
				...
				else if (linha[i].opcode == "sw"){ //se for sw
				...
				else if (linha[i].opcode == "lw"){ //se for lw
				...
				if (linha[i].opcode == "lw"){
					...
					if (linha[i].resultado == linha[j].resultado){ 
...
//MONTAR O PIPELINE
	...
	cout<<endl<<linha[3].dependencia;
	cout<<endl<< linha[3].operador1;

	cout<<endl<< linha[3].resultado;
